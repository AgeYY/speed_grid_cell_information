...
n_time_point_list_long = np.arange(10000, 80000, 5000)
n_time_point_list = np.concatenate((n_time_point_list_short, n_time_point_list_long ))

it_timepoint_file_name = 'it_timepoint_avg_{}_{}_{}_{}_pca{}.hkl'.format(mouse_name, module, day, session, pca_str)
...
it_list_avg = []
manifold_list_avg, manifold_label_list_avg = [], []
for n_time_point in n_time_point_list:
    it, mf, mfl = get_is_torus_timepoint_list(Avg_Fitter(), n_time_point)
    it_list_avg.append(it)
    manifold_list_avg.append(mf)
    manifold_label_list_avg.append(mfl)

if rank == 0:
    data = {'it_list_avg': it_list_avg, 'manifold_list_avg': manifold_list_avg, 'manifold_label_list_avg': manifold_label_list_avg, 'n_time_point_list': n_time_point_list}

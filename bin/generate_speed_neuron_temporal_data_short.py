...
speed_bin_range = [0.025, 0.3]
ci = 0.95
n_shuffle = 2

speed_temporal_file_name = 'speed_temporal_short_{}_{}_{}_{}.npz'.format(mouse_name, module, day, session)
...
# thresh by speed
idx = (speed > speed_bin_range[0]) * ( speed <= speed_bin_range[1])
fire_rate, label = fire_rate[idx], label[idx]
# subsampling
frac = 1.0 / 6
mask = np.random.rand(label.shape[0]) < frac
fire_rate, label = fire_rate[mask], label[mask]
# get the ratemap

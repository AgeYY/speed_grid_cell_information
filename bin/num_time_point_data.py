...
n_boot = 10 # number of bootstrap to compute the probability of sucessing find a torus
n_time_point_list_short = np.arange(100, 3000, 100)
n_time_point_list_long = np.arange(10000, 70000, 10000)
n_time_point_list = np.concatenate((n_time_point_list_short, n_time_point_list_long ))

it_timepoint_file_name = 'it_timepoint_none_{}_{}_{}_{}_pca{}.hkl'.format(mouse_name, module, day, session, pca_str)
...
def get_is_torus_timepoint_list(fitter, n_time_point, rank=0, size=1):
    ...
    for _ in range(rank, n_boot, size):
        ...
        it = itoruser.is_torus(feamap_boot, label_boot, output_manifold=False)
...
for n_time_point in n_time_point_list:
    it = get_is_torus_timepoint_list(None, n_time_point)
    it_list_none.append(it)

if rank == 0:
    data = {'it_list_none': it_list_none, 'n_time_point_list': n_time_point_list}
